...
>
> 最后更新：2026-05-27（内蒙古 v2 数据接入：SQL 取数 + 5 lag-bucket 严格防泄漏 + 4 算法 × 2 freq 全量重跑 + 储能重评）
>
> **2026-05-27 重大变更（仅影响内蒙古）**：
> - 数据源切换 `dws_neimeng_*` → 内蒙现货 SQL 取数 daily 表（`runs/data/neimeng/sqllab_*.csv`），按 D-1 09:30 取数时刻严格按列设定 `window_lag_days`，剔除 `grid_spot_rt_clearing_price`（半天 0 填充） — 见 `doc/内蒙SQL取数接入设计_V1.0.md`。
> - **修复 v1 数据下 LightGBM-Baseline / TwoStage 的 lag=1d 出清价泄漏**：v1 配置把 `CLEARING_DA`（实际 lag=3d）/ `CLEARING_RT`（lag=2d）当 D-1 历史特征用，新 5 lag-bucket 设计下每个 group 按 `window_lag_days` 单独 shift。**修复后 MAE 上升 6~24 元/MWh，但这才是真实生产部署精度**。
> - Conv2D / ResConv2D 按列级 `first_valid_index` 截断训练窗口（方案 B），有效 D 起点 2025-04-22，训练 279 天 / 测试 81 天。
> - 重庆 / 江苏数据未变，结果保持 2026-05-26 口径。
...
|------|-----------|---------|---------------|-------------------|-------------|
| 内蒙古 (neimeng) | 2026-01-27 | 2026-04-17 | 1944 | 7776 | `rt_node_price_nm_hongjing_sta_220kv_1m`（红井 220kV 1M 节点价，v2 命名）|
...
|------|-------------|-------------------|-------------------|-------------------|-----------|------|
| 内蒙古* | 红井节点价 | 158.86 | **118.02** | 134.54 | 135.01 | TwoStage |
| 重庆 | 日前出清价 | 95.47 | 92.46 | **89.17** | — | Conv2D (−6.6% vs Baseline) |
| 江苏 | 江南节点价 | 83.91 | **74.08** | 90.18 | — | TwoStage |

> *内蒙古行为 **v2 数据 + 5 lag-bucket** 重跑（2026-05-27）。v1 时期数字（Baseline 134.19 / TwoStage 107.96 / Conv2D 113.12 / ResConv2D 120.01）因 LightGBM 端 lag=1d 出清价泄漏而偏低，v2 修复后 MAE 上升 10~25 元/MWh 是真实生产部署精度。
> TwoStage 2026-05-26 起改为单次训练-预测（弃用 CV 多折），详见 §3.2 / §4.6。
> ResConv2D（v25 移植）2026-05-26 引入，内蒙单市场（aggressive depth + 80 epochs），其他市场列待补；详见 §3.6 / §4.7。

### 2.2 RMSE 对比（元/MWh）

| 市场 | LightGBM-Baseline | LightGBM-TwoStage | Conv2D-MultiTask | ResConv2D | 最优 |
|------|-------------------|-------------------|-------------------|-----------|------|
| 内蒙古* | 187.80 | **166.72** | 177.39 | 186.46 | TwoStage |
...
| 市场 | LightGBM-Baseline | LightGBM-TwoStage | Conv2D-MultiTask | ResConv2D | 最优 |
|------|-------------------|-------------------|-------------------|-----------|------|
| 内蒙古* | 0.5769 | 0.5361 | **0.7063** | 0.6775 | Conv2D |
| 重庆 | 0.0597 | 0.0432 | **0.2015** | — | Conv2D |
| 江苏 | 0.5532 | 0.6592 | **0.7114** | — | Conv2D |

> *内蒙古 v2 数据 (2026-05-27)；v1 时期 Conv2D=0.7191 / Baseline=0.6468，v2 修正后 Profile Corr 略降但 Conv2D 仍是三算法最强。
...
|------|--------------|--------------|------------------------|---------|------------------|
| 内蒙古* | 187.10 | 211.06 | **170.00** | TwoStage 118.02 | **−30.6%** |
| 重庆 | 101.61 | 125.45 | **100.81** | Conv2D 89.17 | −11.5% |
| 江苏 | 75.47 | 79.63 | **68.64** | TwoStage 74.08 | **−7.9%** (仅 7.9%) |

> *内蒙古 v2 (2026-05-27)；naive 数字与 v1 一致（target 列数值未变），最强 ML 从 TwoStage 107.96 升至 118.02，超越零线幅度由 −36.5% 收窄到 −30.6%（仍然显著超过）。
...
3. **内蒙古所有 naive 都远差于 ML**：节点价噪声大、机组组合/阻塞模式复杂，简单 lag 在这里彻底无效，ML 价值得到充分发挥（v2 下 −16% ~ −30%）。
...
|------|-------------------------|-------------------------|-------------------------|------|
| 内蒙古* | 174.63 | **126.75** | 146.32 | TwoStage |
...
| 市场 | LightGBM-Baseline-15min | LightGBM-TwoStage-15min | Conv2D-MultiTask-15min | 最优 |
|------|-------------------------|-------------------------|-------------------------|------|
| 内蒙古* | 211.75 | **186.17** | 203.19 | TwoStage |
...
| 市场 | LightGBM-Baseline-15min | LightGBM-TwoStage-15min | Conv2D-MultiTask-15min | 最优 |
|------|-------------------------|-------------------------|-------------------------|------|
| 内蒙古* | 0.5376 | 0.5126 | **0.6143** | Conv2D |
| 重庆 | **0.1734** | 0.1509 | 0.1661 | Baseline (差异 0.02) |
| 江苏 | 0.4820 | 0.5884 | **0.6902** | Conv2D |

> *内蒙古 v2 (2026-05-27)。v1 时期 Baseline 138.18 / TwoStage 120.10 / Conv2D 126.03，v2 修复 lag 泄漏后 MAE 上升 6~36 元/MWh。Conv2D Profile Corr 仍排第一。
...
|------|------|--------|-----------|--------|-----|
| 内蒙古* | Baseline | 158.86 | 174.63 | +15.77 | +9.9% |
| 内蒙古* | TwoStage | 118.02 | 126.75 | +8.73 | +7.4% |
| 内蒙古* | Conv2D | 134.54 | 146.32 | +11.78 | +8.8% |
...
| 市场 | naive_lag_1d | naive_lag_7d | naive_rolling_7d_mean | 最强 ML | ML vs naive 改善 |
|------|--------------|--------------|------------------------|---------|------------------|
| 内蒙古* | 196.00 | 219.75 | **178.60** | TwoStage 126.75 | **−29.0%** |
...
|------|------|-----|------|---------|--------------|--------|-----------|
| 内蒙古* | 1h | 158.86 | 187.80 | 1924.4 | 0.5769 | 59 | 35 |
| 内蒙古* | 15min | 174.63 | 211.75 | 253.4 | 0.5376 | 62 | 33 |
...
|------|------|-----|------|--------------|--------|---------------|---------------------|----------------------|
| 内蒙古* | 1h | 118.02 | 166.72 | 0.5361 | 201 | 279 / 81 | 187.10 | +36.9% |
| 内蒙古* | 15min | 126.75 | 186.17 | 0.5126 | 203 | 279 / 81 | 196.00 | +35.3% |
...
|------|------|-----|------|--------------|---------|----------|-------------|--------------|----------------|
| 内蒙古* | 1h | 134.54 | 177.39 | 0.7063 | 0.5102 | 6528 | 30 / 12 | 121.2 | 288.0 / 322.2 |
| 内蒙古* | 15min | 146.32 | 203.19 | 0.6143 | 0.4341 | 26112 | 30 / 12 | 115.7 | 288.0 / 334.3 |
...
> **注（2026-05-27）**：本节内蒙古行为 **v1 数据时期** 早停模式（`_es` 目录），未跟随 v2 数据切换重跑。v2 数据下 80 epochs 跑满版本（即 §3.3 主表）已在 ep5 触底（best_val 112.2），后续 epochs 都是过拟合，早停的边际收益已不明显；优先级低于其他实验项。重庆/江苏 _es 不受影响。
...
|------|------|-----|------|--------------|---------|----------|---------|---------|-----------|
| 内蒙古 (v1) | 1h | **105.31** | 153.37 | 0.7487 | 0.5245 | 89.52 | 1 | 11 | **−7.81** |
| 内蒙古 (v1) | 15min | **119.83** | 175.96 | 0.7063 | 0.5022 | 105.87 | 0 | 10 | **−6.19** |
...
|------|------|-------|--------|-----|------|--------------|----------|-------------|---------------------|
| 内蒙古* | 1h | aggressive | 2.53M | **135.01** | 186.46 | 0.6775 | 6528 | 30 / 24 | 112.2 / ep5 |
| 内蒙古* | 15min | aggressive | — | **—** | — | — | — | 30 / 24 | — |

> *内蒙古 v2 (2026-05-27)。v1 时期 1h MAE=120.01，v2 修正后 +15.00。
> 15min 因 `H_SLOTS=24` 在 (1×5) 卷积输入下触发 PyTorch `Kernel size > input size`，几何上不可用 — v25 原版与本次 v2 均如此（与 v1 一致）。

**与已有算法的位置（内蒙 1h，v2 数据）**：

| 算法 | MAE | RMSE | profile_corr | 参数量 |
|---|---:|---:|---:|---:|
| naive_rolling_7d_mean | 170.00 | 218.66 | 0.40 | 0 |
| LightGBM-Baseline | 158.86 | 187.80 | 0.5769 | 树模型 |
| LightGBM-TwoStage | **118.02** | **166.72** | 0.5361 | 树模型 |
| Conv2D-MultiTask | 134.54 | 177.39 | **0.7063** | ~0.19M |
| **ResConv2D-aggressive** | 135.01 | 186.46 | 0.6775 | 2.53M |

**v2 数据下观察（2026-05-27）**：

1. **TwoStage 仍稳居第一**（MAE 118.02），ResConv2D / Conv2D 几乎打平（135.01 vs 134.54），均显著优于 Baseline (158.86) 与所有 naive。
2. **Conv2D Profile Corr 0.7063 全场最高**：节点价的高频随机性下，端到端时空学习捕捉形态的能力高于树模型工程特征。
3. **ResConv2D 2.53M 参数在 v2 缩短的 ~6.5k 样本上更易过拟合**：best_ep 落在 5（v1 时是 ep45），val_mae 早期触底后训练集继续拟合至 ~4.8，test 反弹至 135。比 Conv2D-MultiTask 多 13× 参数但 MAE 几乎持平，提示 v2 训练样本规模偏小时小网络更合适。
4. **储能 hourly 平铺净收益 1701.7 万 / 81 天（兑现率 64.07%）**：介于 naive_rolling（1699.3）与 Conv2D（1713.9）之间，详见 §8bis。
...
1. **内蒙古**（target=红井节点价，v2 数据 2026-05-27）：TwoStage 仍优于 Baseline（118.02 vs 158.86，−25.7%），Conv2D / ResConv2D 几乎打平（134.54 / 135.01）排 #2 / #3。v1 → v2 修复 lag 泄漏后 MAE 整体上升 10~25 元/MWh，TwoStage 对 Baseline 的优势绝对值（−40.84 元）反而拉大，说明 TwoStage 的工程特征体系在去除泄漏后边际价值仍然显著。
...
> **注（2026-05-27）**：本节内蒙古相关分析（条目 2/3/5）部分数字引用 v1 数据时期值，v2 数据下内蒙 Conv2D 1h MAE=134.54 / Profile Corr=0.7063（详见 §3.3 与 §2 表），结论保持有效（Conv2D 仍三市场 Profile Corr 第一、内蒙古弱于 TwoStage 等），但具体数值以 §2 / §3.3 为准。
...
|------|------|--------------:|----------------|-----------------|---------------|------------------|
| 内蒙古* | 1h | 170.00 | +6.6% | +30.6% | +20.9% | (v1 +38.1%) |
| 内蒙古* | 15min | 178.60 | +2.2% | +29.0% | +18.1% | (v1 +32.9%) |
...
2. **内蒙节点价 4 个 ML 全部超越零线**（v2 数据）：Baseline +6.6%、TwoStage +30.6%、Conv2D +20.9%、ResConv2D +20.6%。v1 → v2 修复 lag 泄漏后 Baseline 接近零线（v1 +21.1% → v2 +6.6%），表明 v1 时 Baseline 的"超零线"幅度有相当部分来自数据泄漏。
...
5. **v2 数据下内蒙古 Baseline 的"超零线"边际价值显著缩水**（+21.1% → +6.6%），但 TwoStage / Conv2D / ResConv2D 仍然显著超越（+20% ~ +31%），说明 LightGBM-Baseline 在 v1 时被泄漏掩盖了"实质上比 naive 强不了多少"的事实。
...
> **2026-05-27 后续发现（仅影响内蒙 v2 数据源）**：v1 时代 `lightgbm_baseline.config.LAG1_GROUPS = (CLEARING_DA, CLEARING_RT)` 把日前出清价 / 实时类参考价当作 D-1 历史特征用，但内蒙 v2 SQL 取数确认这些列的实际数据可见性是 lag=3d（DA）/ lag=2d（RT），存在 1~2 天泄漏。`lightgbm_twostage.features._add_price_lag_features` 同样按 [1,2,7] 取 `dates[i-lag_d]`，也存在同样泄漏。**v2 数据接入时已通过新 5 lag-bucket 设计修复**：每个 group 按 yaml 中显式 `window_lag_days` 单独 shift（baseline）/ clamp lag 偏移（twostage），所有 lag 都 ≥ 该 group 的 `window_lag_days`。修复后内蒙 1h LGBM-Baseline MAE 134.19 → 158.86（+24.67，反映真实精度）。**此泄漏只发生在 v1 内蒙古口径**：重庆 / 江苏 yaml 中 CLEARING_DA / CLEARING_RT 的 `window_lag_days` 为 1d，与 v1 LAG1 假设一致，未受影响。下表的 H1a / H1b / H2-H5 假设结论保持有效，与 v2 修复无矛盾——v2 修复的是"列级数据可见性"而非"特征构建逻辑泄漏"。
...
|------|--------------|------------------|-------------------|-------------------|------------------|-------------|
| 内蒙古* 净收益(万) | 1622.3 | 1679.4 | 1713.6 | **1776.3** | 1678.5 | 2655.9 |
| 内蒙古* 兑现率 | 61.1% | 63.2% | 64.5% | **66.9%** | 63.2% | 100% |
...
> *内蒙古 v2 (2026-05-27)。v1 时期 Baseline=1796.9 / TwoStage=1836.7 / Conv2D=1741.6，因预测 MAE 上升 6~36 元/MWh，决策端净收益相应回落 60~83 万；TwoStage 仍排第一，差距 vs Conv2D 由 +95.1 升至 +97.7 万。naive 数字与 v1 一致。
...
|------|--------------|------------------|-------------------|-------------------|------------------|-----|
| 内蒙古*(万) | 384.0 | 441.1 | 475.2 | **537.9** | 440.2 | 1417.6 |
| 重庆(万) | −15.1 | −9.4 | −49.7 | −50.5 | −37.8 | 125.6 |
| 江苏(万) | 209.0 | 272.8 | 243.0 | 272.1 | **325.4** | 586.2 |

### 8.3 分析要点（v2 数据 2026-05-27）

1. **内蒙古 TwoStage 仍占榜首**：MAE 最优（126.75）储能含补偿净收益也最高（1776.3 万 / 66.9% 兑现率）；v1 → v2 净收益从 1836.7 回落到 1776.3 万（−60.4 万），但仍保持第一名。
2. **MAE 最低 ≠ 储能收益最高（更明显）**：v2 下内蒙 LightGBM-Baseline (MAE 174.63) 储能收益 1713.6 万，反超 Conv2D (MAE 146.32, 1678.5 万)。MAE 差 28 元/MWh，但储能差仅 35 万 — 说明点预测精度与套利决策价值在 v2 下进一步分离，决策端对 LightGBM 的"地板价识别 + 工程特征"更友好。
3. **朴素策略并不总是最差**：内蒙 `naive_rolling_7d_mean`（1679.4 万）v2 下仍介于 Conv2D（1678.5 万）与 LGBM-Baseline（1713.6 万）之间——基本与 Conv2D 打平。
4. **重庆全体套利净利润为负**：节点价波动下预测驱动 MILP 难以仅靠价差套利（补偿前），但容量补偿（280 元/MWh×放电量）使 with_comp 仍为正；PF 套利上限仅 125.6 万，预测策略兑现率 77~82%。
5. **§9.4 决策指标**：各算法 `metrics.json` 含 `decision_metrics.arbitrage` / `with_comp` 各 9 项（VaR/CVaR/最大回撤/Regret/Sharpe 等）。
...
|------|--------------|------------------|-------------------|-------------------|------------------|-----------|-------------|
| 内蒙古* 净收益(万) | 1657.3 | 1699.3 | 1674.3 | 1712.7 | **1713.9** | 1701.7 | 2655.9 |
| 内蒙古* 兑现率 | 62.4% | 64.0% | 63.0% | 64.5% | **64.5%** | 64.1% | 100% |
...
> *内蒙古 v2 (2026-05-27)。v1 时期 Baseline=1789.3 / TwoStage=1815.9 / Conv2D=1813.7 / ResConv2D=1743.3。
> **v2 下 hourly 平铺最优变成 Conv2D-MultiTask（1713.9 万 / 64.5%）**，TwoStage 紧随其后（1712.7 万），差距仅 1.2 万。
...
|------|--------------|------------------|-------------------|-------------------|------------------|-----------|-----|
| 内蒙古*(万) | 418.9 | 461.0 | 436.0 | 474.4 | **475.6** | 463.4 | 1417.6 |
...
| 内蒙古* | LightGBM-Baseline | 1713.6 | 1674.3 | **−39.3** | **−2.3%** |
| 内蒙古* | **LightGBM-TwoStage** | **1776.3** | 1712.7 | **−63.6** | **−3.6%** |
| 内蒙古* | Conv2D-MultiTask | 1678.5 | **1713.9** | **+35.4** | **+2.1%** |
| 内蒙古* | ResConv2D-aggressive | — | 1701.7 | — | — |
...
### 8c.2 分析要点（v2 数据 2026-05-27）

1. **PF 上限完全一致**：三市场三组 hourly PF 与 15min PF 净收益分到分钱都相同，验证决策端实现与预测无关；差异**只来自预测输入**。
2. **内蒙 Conv2D 仍然 hourly > 15min**（+35.4 万 / +2.1%）：与 v1 一致的现象——15min Conv2D 在节点价场景产生高频毛刺，被 MILP 抓到"伪低点充电"；hourly 阶梯平滑后反而更稳。v2 下幅度由 +4.1% 收窄到 +2.1%（hourly 受益于 v2 严格 lag 减小了泄漏带来的低频偏差）。
3. **内蒙 LGBM 系列 v2 也变成 15min > hourly**（Baseline −2.3%, TwoStage −3.6%）：v1 时 LGBM 在 15min/hourly 几乎打平（±1%），v2 修复 lag=1d 出清价泄漏后，15min 原生预测携带的小时内细节对决策端反而更有价值；hourly 平铺丢失这部分信号。
4. **重庆 LGBM 系列 hourly 明显劣化**（−4.8% ~ −7.1%）：15min 原生预测在重庆带来的额外信息被 LGBM 充分利用，hourly 平铺会丢失。
5. **TwoStage / Baseline 在江苏粒度差异微小**（≤0.1%）：江南价的分钟级波动对树模型增益有限。
6. **朴素策略**多数在 ±3% 内波动，方向不一致；说明朴素 lag 本质是低频信号，粒度差异不显著。

### 8c.3 选型建议（v2 更新）

- **内蒙古 1h 预测**：Conv2D 最优（hourly 平铺净收益 1713.9 万）；ResConv2D 紧随（1701.7 万），但比 15min 原生 TwoStage（1776.3 万）仍低 60+ 万。
- **内蒙古 15min 预测**：TwoStage 最优（1776.3 万），全场最高；LGBM-Baseline 次之（1713.6 万）。
- **重庆**：LGBM 系列必须 15min；其他算法粒度敏感性低。
- **江苏**：Conv2D 必须 15min。
- **统一口径**：若全公司只能跑一组，**15min 原生预测**对内蒙古 TwoStage 是绝对最优口径（决策净收益全场峰值）。
...
| 日期 | 内容 |
|------|------|
| 2026-05-27 | **内蒙古 v2 数据接入 + LightGBM 数据泄漏修复 + 全量重跑**：①数据源切换内蒙现货 SQL 取数 daily（`runs/data/neimeng/sqllab_*.csv`），按 D-1 09:30 取数时刻严格按列设定 `window_lag_days`，方案 B 训练区间约 398 天（D 起 2025-04-22），见 `doc/内蒙SQL取数接入设计_V1.0.md`。②**关键修复**：v1 配置把 `CLEARING_DA`(实际 lag=3d) / `CLEARING_RT`(2d) / `CLEARING_RT_NODAL`(4d) 错误地按 lag=1d shift 当历史特征 → 改造 `lightgbm_baseline.config.py / features.py` + `lightgbm_twostage.config.py / features.py`，每个 group 按 `window_lag_days` 单独 shift（baseline）/ `dates[i - lag_d]` 按 group_lag_days clamp（twostage），共 6 个文件、约 200 行变更。③Conv2D / ResConv2D `data.py` 加 `_shift_by_window_lag` + `_stream_cutoff_date`，按列级 `first_valid_index` 截断训练窗口。④全量重跑 4 算法 × 2 freq + naive × 3 + 储能 hourly/15min 双口径。**预测 MAE 变化**（内蒙 1h）：Baseline 134.19→158.86 (+24.67)，TwoStage 107.96→118.02 (+10.06)，Conv2D 113.12→134.54 (+21.42)，ResConv2D 120.01→135.01 (+15.00) — 这是修复泄漏后的真实生产精度。**储能 hourly 平铺最优换人**：v1 TwoStage 1815.9 → v2 Conv2D 1713.9 (差距收窄)；15min 原生 TwoStage 仍排第一 (1776.3 万)。ResConv2D 15min 与 v1 一致仍因 H_SLOTS=24 几何约束失败。详见 §1 / §2 / §2bis / §3 / §8 / §8bis / §8c |
