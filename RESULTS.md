...
>
> 最后更新：2026-05-25（追加 Conv2D-MultiTask）
...
|------|-------------------|-------------------|-------------------|------|
| 内蒙古 | 98.51 | **82.76** | 100.65 | TwoStage |
| 重庆 | 94.93 | 110.47 | **90.57** | Conv2D (-4.6% vs Baseline) |
| 江苏 | 82.60 | **53.36** | 88.25 | TwoStage |

### 2.2 RMSE 对比（元/MWh）

| 市场 | LightGBM-Baseline | LightGBM-TwoStage | Conv2D-MultiTask | 最优 |
|------|-------------------|-------------------|-------------------|------|
| 内蒙古 | 124.11 | **113.41** | 134.98 | TwoStage |
| 重庆 | 169.62 | **144.92** | 165.10 | TwoStage |
| 江苏 | 103.80 | **72.52** | 113.59 | TwoStage |

### 2.3 Profile Correlation 对比

| 市场 | LightGBM-Baseline | LightGBM-TwoStage | Conv2D-MultiTask | 最优 |
|------|-------------------|-------------------|-------------------|------|
| 内蒙古 | **0.7838** | 0.7494 | 0.7714 | Baseline |
| 重庆 | 0.1724 | 0.2271 | 0.2053 | TwoStage |
| 江苏 | 0.5500 | **0.7545** | 0.7257 | TwoStage |
...
|------|-------------------------|-------------------------|-------------------------|------|
| 内蒙古 | 108.66 | **92.52** | 103.74 | TwoStage |
| 重庆 | 95.64 | 109.69 | **90.87** | Conv2D (-5.0% vs Baseline) |
| 江苏 | 88.43 | **54.66** | 93.28 | TwoStage |

### 2bis.2 RMSE 对比（元/MWh）

| 市场 | LightGBM-Baseline-15min | LightGBM-TwoStage-15min | Conv2D-MultiTask-15min | 最优 |
|------|-------------------------|-------------------------|-------------------------|------|
| 内蒙古 | 135.68 | **125.96** | 141.43 | TwoStage |
| 重庆 | 175.88 | **151.49** | 172.55 | TwoStage |
| 江苏 | 110.67 | **76.26** | 121.97 | TwoStage |

### 2bis.3 Profile Correlation 对比

| 市场 | LightGBM-Baseline-15min | LightGBM-TwoStage-15min | Conv2D-MultiTask-15min | 最优 |
|------|-------------------------|-------------------------|-------------------------|------|
| 内蒙古 | **0.7640** | 0.7111 | 0.7584 | Baseline |
| 重庆 | 0.0010 | **0.2758** | 0.0161 | TwoStage |
| 江苏 | 0.5402 | **0.7227** | 0.6834 | TwoStage |
...
| 内蒙古 | Conv2D | 100.65 | 103.74 | +3.09 | +3.1% |
| 重庆 | Baseline | 94.93 | 95.64 | +0.71 | +0.7% |
| 重庆 | TwoStage | 110.47 | 109.69 | −0.78 | −0.7% |
| 重庆 | Conv2D | 90.57 | 90.87 | +0.30 | +0.3% |
| 江苏 | Baseline | 82.60 | 88.43 | +5.83 | +7.1% |
| 江苏 | TwoStage | 53.36 | 54.66 | +1.30 | +2.4% |
| 江苏 | Conv2D | 88.25 | 93.28 | +5.03 | +5.7% |

**观察**：
- 三市场中 **江苏 TwoStage 的 15min 退化最小（+2.4%）**，说明该市场的 D-1 强信号（boundary reserve + 双节点历史价）对 15min 内部波动具备充分解释力。
- **内蒙古退化最大（+10~12%）**（LGBM 类），反映其 15min 内部价格波动结构对当前 boundary 特征体系而言信息不足。
- **重庆 15min ≈ 1h**：重庆数据的 15min 内变化弱（绝大部分小时内 4 个值变化幅度极小），所以两种粒度下模型几乎学到同样的信息。
- **Conv2D 15min 退化普遍最小**（内蒙 +3.1%，重庆 +0.3%，江苏 +5.7%）：本质上 Conv2D 模型同样的网络处理 (C, H_SLOTS, 7) 张量，1h 与 15min 仅差 H_SLOTS 取 12 个连续小时槽 vs 12 个连续 15min 槽的差别，其余架构、容量、训练流程完全一致，因此对粒度切换不敏感。
...
- **代码**：`algorithms/conv2d_multitask/`（移植自 `neimeng_prj/src/model_v8_multitask.py`）
- **方法**：3 层 Conv2d + BatchNorm + GELU + MaxPool 共享骨干，回归头（L1）+ 方向分类头（CrossEntropy，λ=0.3）联合训练
  - 输入张量 `(B, C, H_SLOTS, LOOKBACK_DAYS=7)`，C 由各市场 lag0+时间编码+lag1+lag2 拼成（内蒙/重庆 21，江苏 24）
...
- **参数**：AdamW lr=1e-3，weight_decay=1e-4，warmup 10 ep + Cosine 70 ep，bs=64，epochs=80，191k/193k 模型参数
...
|------|------|-----|------|--------------|---------|----------|-------------|--------------|----------------|
| 内蒙古 | 1h | 100.65 | 134.98 | 0.7714 | 0.5437 | 8736 | 21 / 12 | 78.11 | 288.7 / 290.1 |
| 内蒙古 | 15min | 103.74 | 141.43 | 0.7584 | 0.8609 | 34944 | 21 / 12 | 81.89 | 288.7 / 296.3 |
| 重庆 | 1h | 90.57 | 165.10 | 0.2053 | 0.5040 | 2568 | 21 / 12 | 19.60 | 423.6 / 33.9 |
| 重庆 | 15min | 90.87 | 172.55 | 0.0161 | 0.3973 | 10272 | 21 / 12 | 22.33 | 423.6 / 40.3 |
| 江苏 | 1h | 88.25 | 113.59 | 0.7257 | 0.6168 | 1104 | 24 / 12 | 37.96 | 306.4 / 99.9 |
| 江苏 | 15min | 93.28 | 121.97 | 0.6834 | 0.4746 | 4416 | 24 / 12 | 38.69 | 306.4 / 102.2 |

> **best_val_MAE 与 test_MAE 的显著 gap**（江苏 38 vs 88、重庆 19.6 vs 90.6、内蒙 78 vs 100）反映三件事：
...
1. **重庆是 Conv2D 唯一显著胜出的市场**（1h MAE 90.57 vs Baseline 94.93 vs TwoStage 110.47）：重庆训练集仅 ~115 天，对 LightGBM-TwoStage 的 170+ 特征体系而言数据偏少，而 Conv2D 通过 (C=21, 12, 7) 张量直接学时空模式，对小样本更鲁棒。
...
3. **Profile Correlation 始终不差**：内蒙 1h corr=0.77、江苏 1h corr=0.73，与 LGBM 类持平，说明 Conv2D 把握得住"形状"（波形），主要的 gap 在均值/极端值的标定。
4. **方向分类头作为辅助 loss 的价值**：内蒙 15min 模式下 dir_acc 高达 0.86，但 1h 仅 0.54；推测因 15min 相邻槽方向连续性更强、训练样本更多。重庆 dir_acc 始终 ~0.5，与其低 Profile Corr 一致——本身价格变化弱，方向分类无信号可学。
5. **粒度切换稳健性最高**：见 §2bis.4，Conv2D 三市场 1h→15min 平均退化仅 +3%，远低于 LGBM 类（+10%）。原因是 H_SLOTS 在两种粒度下都取 12，模型容量/感受野不变。
6. **过拟合与未完成的优化**：
   - 当前 80 epoch 后直接用最终模型（无 best-val restore），见 §3.3 末尾说明。
   - 训练曲线显示 epoch 20+ 后 val MAE 不再下降甚至反弹（典型的内蒙古 15min：epoch 0 val=81.9，epoch 79 val=93.8，best=81.9）。
